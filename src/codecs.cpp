...
static inline auto f32_to_bf16(float v) -> std::uint16_t {
  std::uint32_t x;
...
static inline auto bf16_to_f32(std::uint16_t v) -> float {
...
auto Bfloat16Codec::decode_one(std::size_t idx) const -> float {
  return bf16_to_f32(data_[idx]);
}

auto Bfloat16Codec::serialize() const -> std::vector<std::uint8_t> {
  std::vector<std::uint8_t> out(data_.size() * sizeof(std::uint16_t));
  std::memcpy(out.data(), data_.data(), out.size());
  return out;
}

auto Bfloat16Codec::deserialize(const std::uint8_t *data, std::size_t size)
  ...
  for (std::size_t t = 0; t < n_tiles; ++t) {
    ...
    if (range > 0.0f) {
      ...
      for (std::size_t i = lo; i < hi; ++i) {
        ...
        const auto qi = static_cast<std::uint32_t>(
...
auto Uint16Codec::serialize() const -> std::vector<std::uint8_t> {
...
auto Uint16Codec::deserialize(std::vector<std::size_t> tile_offsets,
                              const std::uint8_t *data, std::size_t size)
...
class ZfpCodec : public Codec {
 public:
  ...
  float decode_one(std::size_t idx) const override {
    ...
    zfp_decode_block_float_1(zfp_, buf.data());
    return buf[lane];
  }
  ...
  void decode_gather(const std::size_t *indices, std::size_t k,
    ...
    std::array<float, 4> blk;
    auto last_block = static_cast<std::size_t>(-1);
    for (std::size_t i = 0; i < k; ++i) {
      const std::size_t block = indices[i] >> 2;
      const std::size_t lane = indices[i] & 3;
      if (block != last_block) {
        stream_rseek(stream_, block * bits_per_block_);
        zfp_decode_block_float_1(zfp_, blk.data());
        last_block = block;
      }
      out[i] = blk[lane];
    }
  ...
  auto footprint_bytes() const -> std::size_t override {
    return buffer_.size() + sizeof(*this);
  }
  auto name() const -> const char * override { return "zfp-fixed-rate"; }

  auto serialize() const -> std::vector<std::uint8_t> override {
    ...
    p += sizeof(rate_u);
    const std::array<std::uint64_t, 4> fields = {n_, n_padded_, bits_per_block_,
                                                 buffer_.size()};
    std::memcpy(p, fields.data(), sizeof(fields));
    p += sizeof(fields);
  ...
  static std::unique_ptr<ZfpCodec> deserialize(const std::uint8_t *data,
    ...
    std::memcpy(fields.data(), data + sizeof(rate_u), sizeof(fields));
...
auto zfp_encode_tile(const float *src, std::size_t n, unsigned rate,
...
class AdaptiveZfpCodec : public Codec {
 public:
  ...
  void encode(const float *values, std::size_t n) override {
    ...
    if (has_nan_) {
      ...
      for (std::size_t t = 0; t < n_tiles; ++t) {
        const std::size_t lo = tile_offsets_[t];
        const std::size_t hi = tile_offsets_[t + 1];
        const std::uint8_t state0 = std::isfinite(values[lo])
        ...
        for (std::size_t i = lo; i < hi; ++i) {
          const std::uint8_t s = std::isfinite(values[i])
                                     ? static_cast<std::uint8_t>(1)
                                     : static_cast<std::uint8_t>(0);
          if (s != state) {
    ...
    static constexpr std::array<unsigned, 7> kCandidates = {2, 3,  4, 6,
                                                            8, 12, 16};
    ...
    for (std::size_t t = 0; t < n_tiles; ++t) {
      ...
      for (unsigned rate : kCandidates) {
        ...
        if (bad.size() <= max_outliers ||
            rate == kCandidates[(sizeof(kCandidates) / sizeof(kCandidates[0])) -
                                1]) {
          chosen_rate = rate;
    ...
    std::ranges::sort(outliers, [](const auto &a, const auto &b) {
  ...
  auto decode_one(std::size_t idx) const -> float override {
    auto it = std::ranges::upper_bound(tile_offsets_, idx);
    const std::size_t t =
        static_cast<std::size_t>(it - tile_offsets_.begin()) - 1;
    ...
    if (has_nan_) {
      ...
      const auto p = static_cast<std::uint32_t>(idx - tile_offsets_[t]);
    ...
    if (!outlier_idx_.empty()) {
      auto oit = std::ranges::lower_bound(outlier_idx_,
  ...
  auto serialize() const -> std::vector<std::uint8_t> override;
  static auto deserialize(const class ReducedGrid &grid,
                          const std::uint8_t *data, std::size_t size)
      -> std::unique_ptr<AdaptiveZfpCodec>;
...
auto make_zfp_adaptive(const ReducedGrid &grid, double epsilon,
...
template <typename T>
auto read_array(const std::uint8_t *p, std::vector<T> &out, std::size_t n)
    -> const std::uint8_t * {
  out.resize(n);
...
template <typename T>
auto read_pod(const std::uint8_t *p, T &out) -> const std::uint8_t * {
  std::memcpy(&out, p, sizeof(T));
  return p + sizeof(T);
}
}  // namespace

auto AdaptiveZfpCodec::serialize() const -> std::vector<std::uint8_t> {
...
auto AdaptiveZfpCodec::deserialize(const ReducedGrid &grid,
                                   const std::uint8_t *data, std::size_t size)
    -> std::unique_ptr<AdaptiveZfpCodec> {
...
auto make_uint16_row_tiled(const ReducedGrid &grid) -> std::unique_ptr<Codec> {
  std::vector<std::size_t> offsets(grid.n_rows() + 1);
  offsets[0] = 0;
  for (std::size_t r = 0; r < grid.n_rows(); ++r) {
    offsets[r + 1] = offsets[r] + grid.n_lon(r);
...
auto deserialize_codec(const ReducedGrid &grid, const std::string &name,
                       const std::uint8_t *data, std::size_t size)
    -> std::unique_ptr<Codec> {
